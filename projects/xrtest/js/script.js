...
// COURSES TABLE
d3.csv('assets/xrcourses.csv', function (error,data) {
  console.log(data);

  function tabulate(data, columns) {
        var table = d3.select('#table-container3').append('table')
        var thead = table.append('thead')
        var tbody = table.append('tbody');
        ...
        var cells = rows.selectAll('td')
          ...
          .append('td')
            .text(function (d) { return d.value; });
            // .html(function (d) { return '<a href="'+d.courseURL+'">'+d.value; });

      return table;
    }

    // render the table(s)
    tabulate(data, ['Name','ID', 'School', 'Description']); // 4 column table
